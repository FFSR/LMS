...
public class Ministry {
	...
	@RequestMapping(value="/getAllMinistryData", method=RequestMethod.GET)
	public ResponseEntity<List<LmsMinistry>> getAllMinistry() {
		
		List<LmsMinistry> listLmsministry = new ArrayList<>();
		
		try {
			listLmsministry = lmsMinistryHome.getAllministry();
		
			
			return new ResponseEntity<List<LmsMinistry>>(listLmsministry, HttpStatus.OK);
		}
		catch(Exception ex) {
			ex.printStackTrace();
			return new ResponseEntity<List<LmsMinistry>>(listLmsministry, HttpStatus.EXPECTATION_FAILED);
